    ...
    for r in range(BOARD_ROWS):
        for c in range(BOARD_COLS):
            if board[r][c] == '':
                ...
                score = MinValue(board, alpha, beta)
                # 恢复当前位置为空
                board[r][c] = ''
                # 更新最佳分数
                v = max(score, v)
                # alpha的新取值为当前分数和alpha中的最大值
    ...
    for r in range(BOARD_ROWS):
        for c in range(BOARD_COLS):
            if board[r][c] == '':
                ...
                score = MaxValue(board, alpha, beta)
                # 恢复当前位置为空
                board[r][c] = ''
                # 更新最佳分数
                v = min(score, v)
                # beta的新取值为当前分数和beta中的最小值
                beta = min(beta, score)
                # 如果alpha>=beta，直接剪枝
                if alpha >= beta:
                    return v
    ...
    for r in range(BOARD_ROWS):
        for c in range(BOARD_COLS):
            if board[r][c] == '':
                # 把当前位置设置为AI的棋子
                board[r][c] = 'O'
                # 递归搜索下一层，下一层是玩家的回合，执行最小化搜索，alpha的初始值为负无穷，beta的初始值为正无穷
                score = MinValue(board, -math.inf, math.inf)
                # 恢复当前位置为空
                board[r][c] = ''
                # 如果分数更高，那么走这个位置
                if score > v:
