...
ax[0, 0].set_title('原图')
ax[0, 0].axis('off')
ax[0, 1].imshow(train_label, cmap='gray', vmin=0, vmax=3)
ax[0, 1].set_title('训练标签')
ax[0, 1].axis('off')
ax[1, 0].imshow(true_label, cmap='gray', vmin=0, vmax=3)
ax[1, 0].set_title('真实标签')
ax[1, 0].axis('off')
ax[1, 1].imshow(result_label, cmap='gray', vmin=0, vmax=3)
ax[1, 1].set_title('KNN分类标签')
ax[1, 1].axis('off')
plt.show()
plt.imsave('RsEx/coins_result/coins_KNN_result.png', result_label, cmap="gray")
